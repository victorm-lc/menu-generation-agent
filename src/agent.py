...
from src.configuration import Context
...
async def parse_report(state: VisualBrandingState, runtime: Runtime[Context]):
...
async def menu_content_agent(state: VisualBrandingState, runtime: Runtime[Context]):
    ...
    if state.get("feedback_on_menu_content", "") != "":
        messages = [SystemMessage(content=system_prompt), HumanMessage(content=state.get("feedback_on_menu_content", ""))]
    else:
...
async def menu_content_approval(state: VisualBrandingState):
    ...
    if response.menu_content_approved == True:
        ...
        return {"feedback_on_menu_content": response.feedback_on_menu_content, "menu_content_approved": False}
...
Use your image generation tool to generate a visual menu!"""
    ...
    if state.get("feedback_on_menu_image", "") != "":
        messages = [SystemMessage(content=system_prompt), HumanMessage(content=state.get("feedback_on_menu_image", ""))]
    else:
        messages = [SystemMessage(content=system_prompt), state["messages"][-1]]
